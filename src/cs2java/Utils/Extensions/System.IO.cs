...
namespace System.IO
{
    ...
    internal static class Extensions
    {
        ...
        public static string CreateRelativePathTo(this DirectoryInfo dir, FileSystemInfo file)
        {
            ...
            {
                return file.FullName.ReplaceFirst(dir.FullName + JMeta.DirectorySeparator, "", StringComparison.InvariantCultureIgnoreCase);
            }
            else
            {
            ...
            Action<FileInfo, FileInfo, Dictionary<string, string>> transformer, Dictionary<string, string> args)
        {
            ...
            foreach (var file in files)
            {
                if (file.FullName.Contains(JMeta.DirectorySeparator + ".svn" + JMeta.DirectorySeparator))
                    continue;
